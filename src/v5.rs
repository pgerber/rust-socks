...
impl Socks5Stream {
              ...
              U: ToTargetAddr
    {
        ...
        if selected_method != auth.id() && selected_method != 0 {
...
mod test {
    ...
    const PROXY_ADDR_NO_AUTH_ONLY: &str = "127.0.0.1:1080";
    const PROXY_ADDR_PASSWD_ONLY: &str = "127.0.0.1:1081";

    #[test]
    fn google_no_auth() {
        let addr = "google.com:80".to_socket_addrs().unwrap().next().unwrap();
        let socket = Socks5Stream::connect(PROXY_ADDR_NO_AUTH_ONLY, addr).unwrap();
        google(socket);
    }
    ...
    #[test]
    fn google_dns() {
        let mut socket = Socks5Stream::connect(PROXY_ADDR_NO_AUTH_ONLY, "google.com:80").unwrap();
    ...
    fn bind_no_auth() {
        ...
        let listener = Socks5Listener::bind(PROXY_ADDR_NO_AUTH_ONLY, addr).unwrap();
        bind(listener);
    }
    ...
    fn find_address() -> TargetAddr {
        let socket = Socks5Stream::connect(PROXY_ADDR_NO_AUTH_ONLY, "google.com:80").unwrap();
        socket.proxy_addr().to_owned()
    }

    #[test]
    fn associate_no_auth() {
        let socks = Socks5Datagram::bind(PROXY_ADDR_NO_AUTH_ONLY, "127.0.0.1:15410").unwrap();
    ...
    #[test]
    fn associate_long() {
        let socks = Socks5Datagram::bind(PROXY_ADDR_NO_AUTH_ONLY, "127.0.0.1:15412").unwrap();
    ...
    fn auth_method_not_supported() {
        let addr = "google.com:80".to_socket_addrs().unwrap().next().unwrap();
        let err = Socks5Stream::connect(PROXY_ADDR_PASSWD_ONLY, addr).unwrap_err();
