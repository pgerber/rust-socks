...
impl Socks5Stream {
              ...
              U: ToTargetAddr
    {
        ...
        if auth.id() != buf[1] {
            return Err(io::Error::new(io::ErrorKind::Other, "requested authentication method not offered by server"))
        }
    ...
    fn password_authentication(socket: &mut TcpStream, username: &str, password: &str) -> io::Result<()> {
        ...
        // let mut packet_v1: Vec<u8> = Vec::with_capacity(515);
        // packet_v1.push(1); // version
        // packet_v1.push(username.len() as u8);
        // packet_v1.extend(username.bytes());
        // packet_v1.push(password.len() as u8);
        // packet_v1.extend(password.bytes());

        let mut packet = [0_u8; 515];
        ...
        // assert_eq!(packet_v1[..], packet[..packet_size]);
